  ...
  return (
    <Slider {...settings}>
      {clientData?.map((item, i) => (
        <div
          className={`h-[180px] sm:h-[210px] xl:h-[250px] w-[130px] sm:w-[210px] border border-black flex items-center justify-center hover:rounded-[50px] duration-500 ease-in-out overflow-hidden`}
          key={i}
          onMouseEnter={() => mouseEnter(item,i) }
          onMouseLeave={() => mouseLeave(item,i)}
        >
          
          <img src={`${imageHover ? hoveredIndex ==i? item?.hover_image :item?.banner_image :item?.banner_image }`} alt={`Client ${i}`} className="h-full z-0 object-contain" />
